...
def process_images_gemini():
    ...
    try:
        ...
        for index, file in enumerate(files):
            ...
            # Kiểm tra rate limit cho Gemini API
            if not check_rate_limit():
                results.append({
            ...
            try:
                ...
                # Lưu kết quả
                result_path = os.path.join(session_folder, f"output_{index}_{filename}.txt")
        ...
        with zipfile.ZipFile(zip_path, 'w') as zipf:
            ...
            for i, (file_path, file_name) in enumerate(input_files):
                try:
                    ...
                    logging.info(f"Đã thêm ảnh {i+1} vào ZIP: {new_name}")
                except Exception as e:
                    logging.error(f"Lỗi khi thêm ảnh vào ZIP: {str(e)}")
            
            # Thêm file txt chứa tất cả prompt
            zipf.write(all_prompts_path, "all_prompts.txt")
